...
export default function ReceiptLookupModal({ isOpen, onClose }: ReceiptLookupModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/60 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-2xl bg-white border-2 border-pink-300 rounded-3xl p-4 sm:p-8 shadow-2xl my-auto overflow-hidden">
        {/* Top Garba Ribbon */}
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 text-stone-500 hover:text-maroon-950 hover:bg-pink-50 rounded-full transition cursor-pointer"
        >
          <X className="w-6 h-6" />
        </button>
        ...
        <div className="text-center mb-6 pt-2">
          <div className="w-12 h-12 rounded-full bg-pink-50 border-2 border-pink-200 flex items-center justify-center mx-auto mb-2 text-garba-pink-600 shadow-sm">
        ...
        <form onSubmit={handleSearch} className="space-y-3">
          <div className="flex flex-col sm:flex-row gap-2">
            <input
              ...
              className="flex-1 bg-stone-50 border-2 border-stone-200 rounded-xl px-4 py-3 text-maroon-950 placeholder-stone-400 text-sm focus:outline-none focus:border-garba-pink-500"
              required
            />
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-3 bg-gradient-to-r from-garba-pink-600 via-rose-500 to-pink-500 text-white font-extrabold text-sm rounded-xl hover:brightness-105 transition shadow-sm flex items-center justify-center gap-2 cursor-pointer"
            >
              {loading ? (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3 mb-3 pb-3 border-b border-stone-200">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-base font-serif font-black text-garba-orange-800">
                        {reg.id}
                      </span>
                      <span
                    ...
                    <h4 className="text-lg font-bold text-maroon-950 mt-1">
                      {reg.participantName}
                    </h4>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-garba-pink-600 to-garba-orange-500 text-white font-extrabold text-xs hover:brightness-105 transition shadow-sm cursor-pointer"
                  >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs text-stone-700">
                  <div>
                    ...
                    <strong className="text-maroon-950">{reg.locationName}</strong>
                  </div>
                  <div>
                    <span className="text-stone-500 text-[11px] font-semibold block">Batch Timing:</span>
                    <strong className="text-maroon-950">{reg.batchTime}</strong>
                  </div>
                  <div>
                    <span className="text-stone-500 text-[11px] font-semibold block">Amount Paid:</span>
                    <strong className="text-garba-pink-700 font-extrabold">₹{reg.totalAmount}</strong>
                  </div>
                </div>
                ...
                <div className="mt-3 pt-3 border-t border-stone-200 flex items-center justify-between text-[11px] text-stone-600 font-medium">
                  ...
                  <span className="text-garba-orange-700 font-bold">★ Free Family Pass Included</span>
                </div>
              </div>
            );
          })}
